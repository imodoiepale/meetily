...
pub fn get_default_recordings_folder() -> PathBuf {
    ...
    {
        // Windows: %USERPROFILE%\Music\citywalk-meetings-recordings
        if let Some(music_dir) = dirs::audio_dir() {
            music_dir.join("citywalk-meetings-recordings")
        } else {
            // Fallback to Documents if Music folder is not available
            dirs::document_dir()
                .unwrap_or_else(|| PathBuf::from("."))
                .join("citywalk-meetings-recordings")
        }
    }

    #[cfg(target_os = "macos")]
    {
        // macOS: ~/Movies/citywalk-meetings-recordings
        if let Some(movies_dir) = dirs::video_dir() {
            movies_dir.join("citywalk-meetings-recordings")
        } else {
    ...
    {
        // Linux/Others: ~/Documents/citywalk-meetings-recordings
        dirs::document_dir()
            .unwrap_or_else(|| PathBuf::from("."))
            .join("citywalk-meetings-recordings")
    }
